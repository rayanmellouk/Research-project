...
class ArrivalModel:
    ...
    def __init__(self, price_impact_model:str,dt:float=0.001, **kwargs):
        assert price_impact_model in ['logarithmic','power law'], "price_impact_model must be either 'logarithmic' or 'power law'"
    ...
    def params_check(self,params):
        ...
        if self.price_impact_model == 'logarithmic':
            ...
            assert 'alpha' in params, "Parameter 'alpha' must be provided for power law model"
            assert "beta" in params, "Parameter 'beta' must be provided for power law model"
    ...
    def compute_arrival_probability(self,delta:float):
        ...
        assert 0 <= p <= 1, "Arrival probability must be between 0 and 1"
